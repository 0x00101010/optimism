...
func (oc *OpConductor) loop() {
	...
	for {
		...
		case <-oc.resumeCh:
			...
			// queue an action to make sure sequencer is in the desired state.
			oc.queueAction()
...
func (oc *OpConductor) queueAction() {
	...
	default:
		...
		// Checkout comments in loop() for more details.
	}
}

// handleLeaderUpdate handles leadership update from consensus.
// whenever leadership changed, we delegate the handling to the control loop action.
...
// whenever health status changed, we delegate the handling to the control loop action.
